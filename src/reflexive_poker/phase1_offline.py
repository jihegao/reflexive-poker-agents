        ...
        for regime in ("fixed", "adaptive_shift"):
            for replication in range(5):
                ...
                for hand_index in range(1, CHECKPOINTS[-1] + 1):
                    ...
                    post_switch = regime == "adaptive_shift" and hand_index >= 50
                    active_type = shifted_type if post_switch else initial_type
                    action_distribution = _policy(active_type, hero_image, post_switch)
                    ...
                    ground_truth = {
                        ...
                        "adaptation_probability": 1.0 if post_switch else 0.0,
                        "switch_detected": post_switch,
                    }
                    case_payload = {
...
def model_predictions(
    ...
    budgeted = (
        ...
        if config.provider not in {"baselines", "mock"}
        else None
    )
    rows: list[dict[str, Any]] = []
    for case in cases:
        for treatment in config.treatments:
            if config.provider == "mock":
                ...
                encoded_input = json.dumps(_treatment_view(case, treatment), ensure_ascii=False)
                ...
                provider_response = budgeted.structured(
                    instructions=(
                        "Estimate the simulated opponent's latent type, next-action distribution, "
                    ...
                    state=_treatment_view(case, treatment),
...
def _report(config: OfflineBenchmarkConfig, summary: pd.DataFrame, gate: dict[str, Any]) -> str:
    lines = [
        ...
        "## 汇总",
        "",
        "| Method | Regime | N | Type Brier ↓ | Action Brier ↓ | Regret ↓ |",
        "|---|---|---:|---:|---:|---:|",
    ]
    for row in summary.to_dict(orient="records"):
        lines.append(
            f"| {row['method']} | {row['regime']} | {int(row['n'])} | "
...
def run_offline_benchmark(config: OfflineBenchmarkConfig) -> dict[str, Any]:
    ...
    if config.provider in {"baselines", "mock"}:
        gate["valid"] = True
    manifest_payload = {
        "protocol": "reflexive-poker-phase1-offline-v1",
        "configuration": {
            **asdict(config),
    ...
    token_diagnostics = pd.DataFrame(
        columns=["case_id", "d1_budget_matched_tokens", "recursive_d2_tokens", "ratio"]
    )
    if not llm_scores.empty:
        ...
        if required.issubset(token_pivot.columns):
            token_diagnostics = token_pivot.reset_index()[
                ...
                columns={
                    ReasoningTreatment.BUDGET_MATCHED_D1.value: "d1_budget_matched_tokens",
                    ReasoningTreatment.RECURSIVE_D2.value: "recursive_d2_tokens",
                }
            )
            denominator = token_diagnostics["recursive_d2_tokens"].replace(0, float("nan"))
            token_diagnostics["ratio"] = (
                token_diagnostics["d1_budget_matched_tokens"] / denominator
            )
